...
class IPTest extends \PHPUnit_Framework_TestCase
{
     ...
     */
    public function testAnonymizeIPv4($ipString, $expected)
    {
        ...
        for ($byteCount = 0; $byteCount <= 4; $byteCount++) {
            $result = $mappedIp->anonymize($byteCount);
            $expectedIp = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff" . $expected[$byteCount];
            $this->assertEquals($expectedIp, $result->toBinary(), "Got $result, Expected " . bin2hex($expected[$byteCount]));
        }
        $this->assertEquals("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\x00\x00\x00\x00\x00", $mappedIp->anonymize(5)->toBinary());
    }

    public function getIPv6AddressesToAnonymize()
    {
        return array(
    ...
    /**
     * @dataProvider getIPv6AddressesToAnonymize
     */
    public function testApplyIPMask6($ipString, $expected)
    {
        $ip = IP::fromStringIP($ipString);
